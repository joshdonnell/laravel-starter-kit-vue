...
Document every persisted model attribute with `@property`. Document relationships and read-only accessors with `@property-read`. Include generic key and value types for arrays and collections. Keep these declarations synchronized with the database schema and runtime casts.
